...
augroup endwise " {{{1
  ...
  autocmd FileType julia
        \ let b:endwise_addition = '\=submatch(0)=="{" ? "}" : "end"' |
        \ let b:endwise_words = 'module,type,function,if,while' |
        \ let b:endwise_pattern = '^\s*\zs\%(module\|type\|function\|if\|while\|\|for\)\>\%(.*[^.:@$]\<end\>\)\@!\|\<do\ze\%(\s*|.*|\)\=\s*$' |
          \ let b:endwise_syngroups = 'juliaBlocksItems'
